...
::
::     unreal\render_frame.bat                  :: MRQ_PathTracer (spec condition 2)
::     unreal\render_frame.bat MRQ_LumenHW      :: conditions 3-5
::
...
:: target at the USD prim -- see unreal\setup_bench.py for why there is no
:: Camera Cut track.
::
...
set "OUTDIR=%REPO%\data\bench\unreal\%CONDITION%"
...
if not exist "%OUTDIR%\*.exr" (
    ...
    echo [rendergap]   env\rg-unreal.bat -run=pythonscript -script=unreal\setup_bench.py
